...
def stock_status_pie(enriched: pd.DataFrame) -> go.Figure:
    status_counts = enriched["status"].value_counts().reset_index()
...
def demand_trend(df: pd.DataFrame, product_id: str) -> go.Figure:
    ...
    pdata["7d_avg"] = pdata["daily_demand"].rolling(7).mean()

    fig = go.Figure()
    fig.add_trace(go.Bar(
        x=pdata["date"], y=pdata["daily_demand"],
        name="Daily Demand", marker_color="rgba(52,152,219,0.5)"
    ))
    fig.add_trace(go.Scatter(
        x=pdata["date"], y=pdata["7d_avg"],
        name="7-Day Moving Avg", line=dict(color="#e74c3c", width=2)
    ))
    name = pdata["product_name"].iloc[0]
    fig.update_layout(
        title=f"📈 Demand Trend — {name}",
...
def stock_vs_rop_bar(enriched: pd.DataFrame) -> go.Figure:
    ...
    fig.update_layout(
        title="📦 Current Stock vs Reorder Point vs Safety Stock",
...
def stock_value_treemap(enriched: pd.DataFrame) -> go.Figure:
    fig = px.treemap(
        ...
        labels={"stock_value": "Stock Value ($)", "days_of_stock": "Days of Stock"},
    )
    return fig

def category_demand_bar(df: pd.DataFrame) -> go.Figure:
    cat = df.groupby(["category", "date"])["daily_demand"].sum().reset_index()
    cat_avg = cat.groupby("category")["daily_demand"].mean().reset_index()
    cat_avg.columns = ["Category", "Avg Daily Demand"]
    fig = px.bar(
        cat_avg.sort_values("Avg Daily Demand", ascending=True),
        x="Avg Daily Demand", y="Category", orientation="h",
        title="🏭 Avg Daily Demand by Category",
